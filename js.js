// 本试卷唯一 ID: 17FC07622B70E8B2572044083E7EF2CF, 请勿修改本行内容
/**
...
module.exports = {
    ...
    qn3: () => {
      ...
      // 块级作用域变量提升。 i提升到wiondw全局下， 最后一次赋值为10.
      return B;
    },
    qn4: () => {
      // 以下代码片段, 选择它的打印输出 (省略换行)
      ...
      // 1.setTimeout的第三个参数i可以当做实参传入setTimeout第一个参数函数中。
      // 2.变量提升 timer 每次执行完console，都会把所有timer定时器清除，所以最后一个定时器不会触发。
      return A;
    },
    qn5: () => {
    ...
    qn16: () => {
      // 关于let和var, 以下正确的是:
      //
      // A. let是函数级作用域, var是块级作用域, let和var变量定义都会提升 (hoist)
      //
